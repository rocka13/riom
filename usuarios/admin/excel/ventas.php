...
header("Content-Disposition: attachment; filename=ventas$year.xls");
...
$sql_excel="SELECT paciente.*, cita.*,usuario.nick FROM paciente,cita,usuario
      ...
      AND usuario.id_usuario=cita.responsable AND cita.ff='$year' AND cita.anulado='NO'";
$consulta_excel=$enlace->query($sql_excel);

$sql_gastos="SELECT gastos.*, precio*cantidad AS total_gasto, usuario.nick FROM gastos,usuario WHERE gastos.fecha='$year' 
